...
async def upload_document(file:UploadFile=File(...)):
    ...
    temp_file_path = None
    try:
        # Clear previous documents from knowledge base
        print(f"Clearing knowledge base for new document upload...")
        clear_index()
        ...
        if documents:
            ...
            add_document(full_text_content, document_name=file.filename)
            total_chunks_added=len(documents)
        return DocumentUploadResponse(
            message=f"PDF '{file.filename}' sucessfully uploaded and indexed",
        ...
        #cleaning up temp files after uploading to vector db
...
async def chat_with_agent(request:QueryRequest):
    ...
    try:
        #passing web info in config for the agent to acess
        config={
            "configurable":{"thread_id":request.session_id,"web_search_enabled":request.enable_web_search}
        }
...
async def clear_knowledge_base():
    ...
    try:
        ...
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail=f"Failed to clear knowledge base: {e}")
